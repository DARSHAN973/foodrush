"use client";

// Client Component — needed because status update buttons will use
// server actions with useTransition for optimistic UI later.
// For now the buttons are UI-only placeholders.

import { useState } from "react";

// StatusBadge — see app/vendor/page.js for full explanation.
function StatusBadge({ status }) {
  const config = {
    PLACED: {
      cls: "bg-blue-50 text-blue-700 border-blue-200/60",
      dot: "bg-blue-500",
      label: "Placed",
    },
    CONFIRMED: {
      cls: "bg-indigo-50 text-indigo-700 border-indigo-200/60",
      dot: "bg-indigo-500",
      label: "Confirmed",
    },
    PREPARING: {
      cls: "bg-amber-50 text-amber-700 border-amber-200/60",
      dot: "bg-amber-400",
      label: "Preparing",
    },
    OUT_FOR_DELIVERY: {
      cls: "bg-purple-50 text-purple-700 border-purple-200/60",
      dot: "bg-purple-500",
      label: "Out for Delivery",
    },
    DELIVERED: {
      cls: "bg-green-50 text-green-700 border-green-200/60",
      dot: "bg-green-500",
      label: "Delivered",
    },
    CANCELLED: {
      cls: "bg-red-50 text-red-700 border-red-200/60",
      dot: "bg-red-500",
      label: "Cancelled",
    },
  };

  const item = config[status] || {
    cls: "bg-gray-50 text-gray-700 border-gray-200/60",
    dot: "bg-gray-400",
    label: status,
  };

  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] sm:text-xs font-semibold uppercase ${item.cls}`}
    >
      <span className={`h-1.5 w-1.5 rounded-full ${item.dot}`} />
      {item.label}
    </span>
  );
}

// nextStatus — defines the linear vendor order status flow.
// Vendor can only move an order FORWARD, never backward.
// DELIVERED and CANCELLED are terminal — no further action.
function getNextStatus(current) {
  const flow = {
    PLACED: "CONFIRMED",
    CONFIRMED: "PREPARING",
    PREPARING: "OUT_FOR_DELIVERY",
    OUT_FOR_DELIVERY: "DELIVERED",
  };
  return flow[current] || null;
}

function getNextLabel(current) {
  const labels = {
    PLACED: "Confirm Order",
    CONFIRMED: "Start Preparing",
    PREPARING: "Mark Out for Delivery",
    OUT_FOR_DELIVERY: "Mark Delivered",
  };
  return labels[current] || null;
}

// Hardcoded orders — will be replaced with real DB query after route protection.
const INITIAL_ORDERS = [
  {
    id: 101,
    customerName: "Rahul Sharma",
    items: ["Paneer Butter Masala", "Garlic Naan", "Lassi"],
    total: 580,
    status: "PLACED",
    createdAt: new Date("2025-07-07T13:20:00"),
  },
  {
    id: 102,
    customerName: "Priya Patel",
    items: ["Veg Biryani"],
    total: 220,
    status: "PREPARING",
    createdAt: new Date("2025-07-07T12:45:00"),
  },
  {
    id: 103,
    customerName: "Arjun Mehta",
    items: ["Dal Tadka", "Jeera Rice"],
    total: 410,
    status: "DELIVERED",
    createdAt: new Date("2025-07-07T11:10:00"),
  },
  {
    id: 98,
    customerName: "Sneha Iyer",
    items: ["Chole Bhature", "Mango Lassi", "Gulab Jamun", "Raita"],
    total: 890,
    status: "DELIVERED",
    createdAt: new Date("2025-07-06T19:30:00"),
  },
  {
    id: 95,
    customerName: "Karan Joshi",
    items: ["Masala Dosa", "Filter Coffee"],
    total: 340,
    status: "CANCELLED",
    createdAt: new Date("2025-07-06T18:05:00"),
  },
];

export default function VendorOrders() {
  // Local state — simulates status updates until real server action is wired up.
  // useState holds a copy of orders so button clicks update the UI immediately.
  const [orders, setOrders] = useState(INITIAL_ORDERS);

  // handleAdvance — moves a specific order to its next status in the flow.
  // Filters orders by id and replaces just that order's status.
  function handleAdvance(orderId, nextStatus) {
    setOrders((prev) =>
      prev.map((o) => (o.id === orderId ? { ...o, status: nextStatus } : o))
    );
  }

  return (
    <div className="space-y-6 sm:space-y-8">
      {/* Page Header */}
      <div>
        <h1 className="text-xl sm:text-3xl font-black text-gray-900 tracking-tight">
          Orders
        </h1>
        <p className="mt-1 text-xs sm:text-sm text-gray-500">
          Manage and update the status of your incoming orders.
        </p>
      </div>

      {/* Orders list */}
      <div className="space-y-4">
        {orders.length === 0 ? (
          <div className="rounded-2xl border border-gray-100 bg-white p-12 text-center shadow-sm">
            <p className="text-gray-400 text-sm">No orders yet.</p>
          </div>
        ) : (
          orders.map((order) => {
            const nextStatus = getNextStatus(order.status);
            const nextLabel = getNextLabel(order.status);

            return (
              <div
                key={order.id}
                className="rounded-2xl border border-gray-100 bg-white p-4 sm:p-6 shadow-sm"
              >
                {/* Order header row */}
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <span className="font-mono font-black text-gray-900">
                      #{order.id}
                    </span>
                    <StatusBadge status={order.status} />
                  </div>
                  <span className="text-xs text-gray-400">
                    {order.createdAt.toLocaleDateString("en-IN", {
                      day: "2-digit",
                      month: "short",
                    })}{" "}
                    {order.createdAt.toLocaleTimeString("en-IN", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </span>
                </div>

                {/* Customer + Items */}
                <div className="mt-3 space-y-1">
                  <p className="text-sm font-semibold text-gray-800">
                    {order.customerName}
                  </p>
                  <p className="text-xs text-gray-500">
                    {order.items.join(" · ")}
                  </p>
                </div>

                {/* Footer: Total + Action button */}
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-50 pt-4">
                  <span className="text-base font-black text-gray-900">
                    ₹{order.total.toLocaleString("en-IN")}
                  </span>

                  {/* Only show action button if order can still be advanced */}
                  {nextStatus && nextLabel ? (
                    <button
                      onClick={() => handleAdvance(order.id, nextStatus)}
                      className="rounded-xl bg-orange-600 px-4 py-2 text-xs font-bold text-white hover:bg-orange-700 transition"
                    >
                      {nextLabel}
                    </button>
                  ) : (
                    <span className="text-xs text-gray-400 font-medium">
                      {order.status === "DELIVERED"
                        ? "Order complete"
                        : "Order cancelled"}
                    </span>
                  )}
                </div>
              </div>
            );
          })
        )}
      </div>
    </div>
  );
}
