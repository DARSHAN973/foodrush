...
export async function getAdminRestaurants() {
  const restaurants = await prisma.restaurant.findMany({
    select: {
      ...
      // _count — Prisma aggregate that returns the count of related rows.
      // We use it here to show how many reviews each restaurant has received
      // in the admin panel without fetching all review rows.
      _count: {
        select: { reviews: true },
      },
    },
