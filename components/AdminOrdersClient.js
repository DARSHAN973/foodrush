    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-gray-200 gap-1 overflow-x-auto pb-1 no-scrollbar">
        <button
          ...
          className={`flex items-center gap-1.5 border-b-2 px-3 sm:px-4 py-2.5 text-xs sm:text-sm font-semibold transition whitespace-nowrap ${
            activeTab === "ACTIVE"
        ...
        >
          All Checkouts
          <span className="rounded-full bg-gray-100 px-2 py-0.5 text-[10px] sm:text-xs font-bold text-gray-600">
            {orders.length}
          </span>
        </button>
      </div>
        ...
        <div className="grid gap-6">
          {filteredOrders.map((order) => {
            ...
            return (
              <div
                key={order.id}
                className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition duration-200"
              >
                {/* Card Header (Checkout Details) */}
                <div className="border-b border-gray-100 bg-gray-50/50 px-4 py-3 sm:px-6 sm:py-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="font-mono text-[10px] sm:text-xs font-bold uppercase tracking-wider text-orange-600 bg-orange-50 px-2 py-0.5 rounded-lg">
                        ORDER #{order.id}
                      </span>
                      <span
                        className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-0.5 text-[10px] sm:text-xs font-bold ${config.cls}`}
                      >
                        <span
                    ...
                    <p className="text-[10px] sm:text-xs text-gray-400 font-medium">{orderDate}</p>
                  </div>
                  <div className="flex sm:flex-col justify-between sm:text-right items-center sm:items-end border-t border-gray-100 sm:border-t-0 pt-2 sm:pt-0">
                ...
                <div className="p-4 sm:p-6 grid gap-4 sm:gap-6 md:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-gray-100 bg-gray-50/30 p-3.5 sm:p-4 space-y-3 md:col-span-1">
                    <h3 className="text-[10px] sm:text-xs font-bold uppercase tracking-wider text-gray-400">
                      Delivery Details
                    </h3>
                    <div className="space-y-2 text-xs sm:text-sm">
                      ...
                      <div className="border-t border-gray-100 pt-2">
                        ...
                        <p className="text-gray-600 leading-snug mt-0.5 text-[11px] sm:text-xs">
                      ...
                      <div>
                        ...
                        <p className="font-mono text-gray-700 mt-0.5 text-[11px] sm:text-xs">
                          {order.phone}
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="space-y-3 sm:space-y-4 md:col-span-2">
                    ...
                    <div className="grid gap-3 sm:gap-4">
                      {order.restaurantOrders.map((ro) => (
                        <div
                          key={ro.id}
                          className="flex flex-col justify-between rounded-xl border border-gray-100 p-3 sm:p-4 sm:flex-row sm:items-center gap-3 bg-white"
                        >
                          ...
                          <div className="space-y-1.5">
                            ...
                            <ul className="space-y-1">
                              {ro.items.map((item) => (
                                ...
                                >
                                  <span className="flex h-4.5 w-4.5 items-center justify-center rounded bg-gray-50 text-[10px] font-bold text-gray-500">
                                    {item.quantity}x
                                  </span>
                          ...
                          <div className="flex items-center gap-2 self-start sm:self-center border-t border-gray-50 sm:border-t-0 pt-2 sm:pt-0 w-full sm:w-auto justify-between sm:justify-start">
                            <span className="text-[10px] font-bold text-gray-400 sm:hidden">Fulfillment Status:</span>
                            <div className="flex items-center gap-2">
                              ...
                              <select
                                ...
                                className="rounded-xl border border-gray-250 bg-white px-2.5 py-1.5 text-xs font-bold text-gray-800 shadow-sm focus:border-orange-500 focus:outline-none focus:ring-1 focus:ring-orange-500 disabled:opacity-50 transition cursor-pointer"
                              >
